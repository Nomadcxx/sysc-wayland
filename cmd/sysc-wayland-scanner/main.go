...
func init() {
	...
	flag.StringVar(&xdgShellImport, "xdg-shell-import", "", "Go import path for external xdg-shell types")
}
...
func main() {
	...
	needsXDGImport := requiresXDGImport()
	if needsXDGImport && xdgShellImport == "" {
		log.Fatal("-xdg-shell-import is required for external xdg_* interfaces")
	}
	...
	fmt.Fprintf(w, "// https://github.com/Nomadcxx/sysc-wayland/cmd/sysc-wayland-scanner\n")
	...
	if protocol.Name != "wayland" {
		fmt.Fprintf(w, "import \"github.com/Nomadcxx/sysc-wayland/client\"\n")
	}
	if needsXDGImport {
		fmt.Fprintf(w, "import xdg_shell %q\n", xdgShellImport)
	}
...
func requiresXDGImport() bool {
	for _, iface := range protocol.Interfaces {
		for _, request := range iface.Requests {
			for _, arg := range request.Args {
				if !isLocalInterface(arg.Interface) && strings.HasPrefix(arg.Interface, "xdg_") {
					return true
				}
			}
		}
